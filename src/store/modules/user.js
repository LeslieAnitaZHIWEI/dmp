...
const user = {
  ...
  mutations: {
    ...
    SET_MENU: (state, menu) => {
      menu=menu.filter(item=>item.name!='需求管理').filter(item=>item.name!='消息管理').filter(item=>item.name!='推广管理').filter(item=>item.name!='定向管理').filter(item=>item.name!='财务').filter(item=>item.name!='后台管理').filter(item=>item.name!='投放管理')
      state.menu = menu
      setStore({
        name: 'menu',
        content: state.menu,
